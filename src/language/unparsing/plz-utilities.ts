...
type Options = {
  readonly flow: 'inline' | 'multiline'
  readonly ordinalKeys: 'omit' | 'preserve'
}
...
export const atomOrMoleculeUnparser = (options: Options) => {
  const unparseMolecule = moleculeUnparser(options)
  return (semanticContext: SemanticContext) => (value: Atom | Molecule) =>
    typeof value === 'string' ?
      unparseAtom(value)
...
export const moleculeUnparser =
  (options: Options) => (semanticContext: SemanticContext) => {
    const unparseSugarFreeMolecule = sugarFreeMoleculeUnparser(options)
    const unparseAtomOrMolecule = atomOrMoleculeUnparser(options)

    const context: Context = { unparseAtomOrMolecule, semanticContext }
    const sugar = unparseSugaredExpression(
      context,
      unparseSugarFreeMolecule(context),
    )
      ...
      switch (keyword) {
        ...
        case '@object':
          return sugar(
            value,
            readObjectTypeExpression,
            unparseSugaredObjectType(options),
          )
        case '@union':
        ...
        default: {
          ...
          if (isExpression(potentialKeywordExpression)) {
            const result = unparseSugaredGeneralizedKeywordExpression(context)(
              potentialKeywordExpression,
            )
            return either.flatMapLeft(result, _ =>
              unparseSugarFreeMolecule(context)(value),
            )
          } else {
            return unparseSugarFreeMolecule(context)(value)
          }
        }
      }
    }
...
export const moleculeAsKeyValuePairStrings =
  ...
  (value: Molecule): Either<UnserializableValueError, readonly string[]> => {
...
const sugarFreeMoleculeUnparser = (options: Options) => {
  const moleculeAsKeyValuePairStringsWithOptions =
    moleculeAsKeyValuePairStrings(options)
  return (context: Context) => {
    const moleculeAsKeyValuePairStringsWithContext =
      moleculeAsKeyValuePairStringsWithOptions(context)
    return (value: Molecule) => {
      const { closeBrace, comma, openBrace } = punctuation(styleText)
      if (value.entries.length === 0) {
        return either.makeRight(openBrace + closeBrace)
      } else {
        return either.map(
          moleculeAsKeyValuePairStringsWithContext(value),
...
const unparseSugaredExpression =
  ...
  >(
    ...
    either.match(readSpecificExpression(asSemanticGraph(value)), {
      left: _ => unparseSugarFreeMolecule(value),
      right: unparseSpecificExpression(context),
    })
  ...
  (
    ...
    return option.match(signatureSugarTypeAnnotation(expression), {
      ...
      none: _ =>
        // Unparse using `=>` notation.
        either.flatMap(
          unparseSugaredFunctionParameter(context)(expression),
          parameterAsString =>
            either.flatMap(
              serializeIfNeeded(expression[1].body),
              serializedBody =>
...
const unparseKeyPathOfSugaredIndex =
  ({ unparseAtomOrMolecule, semanticContext }: Context) =>
  (query: ObjectNode): Either<UnserializableValueError, string> => {
      ...
      clauses =>
        either.flatMap(
          ...
          serializedProperties =>
            ...
            : either.flatMap(
                moleculeAsKeyValuePairStrings(options)(context)(
                  serializedProperties,
                ),
...
const unparseSugaredUnion =
  (context: Context) => (expression: UnionExpression) => {
          ...
          either.flatMap(unparsedUnion, unionAsString =>
            either.map(
              either.flatMap(
                ...
                context.unparseAtomOrMolecule(context.semanticContext),
              ),
              memberAsString => {
